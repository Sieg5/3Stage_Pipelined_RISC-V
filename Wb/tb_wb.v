...
module wb_tb;
    ...
    initial begin
        ...
        if (in_file == 0) begin
	       $display("ERROR: Could not find execute_output.txt! Check your working directory.");
           $finish;
        end
     ...
     $fdisplay(out_file, "inst_fetch_pc_o | wb_stall_o | wb_stall_first_o | wb_stall_second_o | wb_write_address_o | wb_write_data_o | wb_write_byte_o | wb_read_data_o | inst_mem_address_o | inst_mem_is_ready_o");
        ...
        while (!$feof(in_file)) begin
            status = $fscanf(in_file, "%h %h %h %h %b %h %b %h %b %b %b %b %b %b %b %b", 
            ...
            if (status == 16) begin
                ...
                @(negedge clk);
                // The formatting uses %16h to ensure 32-bit hex values stay aligned under their headers
