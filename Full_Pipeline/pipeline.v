...
wb wb_stage (
   ...
   // Connecting WB inputs
   .stall_read_i (stall_read),             // Pipe
   .fetch_pc_i (fetch_pc),                 // Pipe
   .wb_branch_i (wb_branch),               // EX
   .wb_mem_to_reg_i (wb_mem_to_reg),       // EX
   .mem_write_i (wb_mem_write),            // Ex
   .write_address_i (write_address),       // Ex - Pipe
   .alu_operand2_i (alu_operand2),         // EX - Pipe
   .alu_operation_i (alu_operation),       // ID
   .wb_alu_operation_i (wb_alu_operation), // EX
   .wb_read_address_i (wb_read_address),   // EX
   .dmem_read_data_i (dmem_read_data),     // Mem
   .dmem_write_valid_i (dmem_write_valid), // Mem

   // Connecting WB outputs
   .inst_mem_address_o (inst_mem_address),   //IMEM (TB)
   .inst_mem_is_ready_o (inst_mem_is_ready), // IMEM (TB)
   .wb_stall_o (wb_stall),                   // Pipe (IF/ID)
   .wb_write_address_o (wb_write_address),   // Mem
   .wb_write_data_o (wb_write_data),         // Mem
   .wb_write_byte_o (wb_write_byte),         // Mem
   .wb_read_data_o (wb_read_data),           // ID
   .inst_fetch_pc_o (inst_fetch_pc),         // IF
   .wb_stall_first_o (wb_stall_first),       // Pipe
   .wb_stall_second_o (wb_stall_second)      // Pipe
);
